...
#
    #And user clicks on play Icon for CBS video
     #Examples: 
      ...
      Scenario Outline: [MUTV]TC008 CBR screen should be displayed when user tries to open the content that is marked behind registration and on login user should be taken to the respective screen,[MUTV]TC013 Verify that CBS/CBR badges display for previous and future schedule program but not for the current program for logout user
    Given user navigates to manu android application
    And user clicks on cancel button in apple id screen
    And user clicks lets go button in screen one in ios
    And user clicks on ask me later in screen two in ios
   ...
   Scenario: [MUTV]TC015 Validate the MUTV hero carousel when disabled from CMS
